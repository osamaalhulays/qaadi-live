...
export function evaluateQN21(text: string): QN21Result[] {
  ...
  return QN21_CRITERIA.map((c) => {
    const matched = c.patterns.filter((p) => {
      const r = new RegExp(p.source, p.flags.replace(/[gy]/g, ""));
      return sentences.some(
        (s) => r.test(s) && !/\b(no|not|without|ignored|lacking|missing|absent)\b/i.test(s)
      );
    }).length;
    const ratio = c.patterns.length ? matched / c.patterns.length : 0;
